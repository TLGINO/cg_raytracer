...
public:
	...
	Hit intersect(Ray ray)
	{
		...
		float D = 0;
		if (cdotc > pow(cdotd, 2))
		{
			D = sqrt(cdotc - pow(cdotd, 2));
		}
		if (D > this->radius)
			return Hit();

		float t_temp = sqrt(pow(this->radius, 2) - pow(D, 2));
		float t1 = cdotd - t_temp;
		float t2 = cdotd + t_temp;
...
int main(int argc, const char *argv[])
{
	...
	for (int i = 0; i < width; i++)
		for (int j = 0; j < height; j++)
		{
			...
			glm::vec3 direction(dx, dy, dz); // fill in the correct values
