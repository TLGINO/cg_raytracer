...
public:
	...
	{
		glm::vec3 d = this->inverseTransformationMatrix * glm::vec4(ray.direction, 0.0);
		glm::vec3 o = this->inverseTransformationMatrix * glm::vec4(ray.origin, 1.0);
		d = normalize(d);
		...
		if (D <= radius)
		{
			...
			hit.distance = glm::distance(o, hit.intersection);
			hit.object = this;
